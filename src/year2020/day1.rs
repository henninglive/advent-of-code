...
//! but somehow, you'll need to find fifty of these coins by the time you arrive so you can pay
...
pub fn part1() -> i64 {
    let numbers = load();
    iproduct!(numbers.iter(), numbers.iter())
        .find(|(&i, &j)| i + j == 2020)
...
pub fn part2() -> i64 {
    let numbers = load();
    iproduct!(numbers.iter(), numbers.iter(), numbers.iter())
...
#[test]
fn test_part1() {
    assert_eq!(part1(), 960075)
}

#[test]
fn test_part2() {
    assert_eq!(part2(), 212900130)
}
